//+------------------------------------------------------------------+
//| ZoneRaider_v4f.mq5                                              |
//| Event-aware · suspends order placement during news windows      |
//|                                                                  |
//| Polls GET /events on each poll cycle. Before placing orders,    |
//| checks whether the current UTC time falls within any event's    |
//| suspend window (event.time ± event.suspendMinutes). If yes,     |
//| placement is deferred until the next poll clears the window.    |
...
//--- Event store (populated from GET /events each poll cycle)
struct TradingEvent
  {
   int   hourUTC;
   int   minuteUTC;
   ...
   if(!g_zonesLoaded)
     {
      ...
      if(IsAfterNYOpen(dt))
         Print("ZoneRaider v4f: WARNING — past NY open, zones not yet loaded");
      PollServer();
      PollEvents();
     }

   //--- Place immediately once zones are available; suspend during event windows
   //    g_ordersPlaced is NOT set while suspended — zones are remembered and retried
   //    every 30 s until the window clears, then placed fresh via normal dedup logic
   if(g_zonesLoaded && g_zoneCount > 0)
     {
      if(IsSuspended())
        {
         Print("ZoneRaider v4f: Event suspend window active — deferring placement");
        }
      else
        {
         PlaceAllOrders();
...
//+------------------------------------------------------------------+
void SwingTrail(ulong ticket) // v4f
  {
...
void PollServer()
  {
   ...
   Print("ZoneRaider v4f: Polling zones ", url);
   int res = WebRequest("GET", url, headers, 5000, post, result, responseHeaders);
   if(res != 200)
     {
      Print("ZoneRaider v4f: Zone poll failed HTTP=", res,
...
void ParseZones(const string json)
  {
   ...
   if(StringFind(json, "{") < 0)
     {
      Print("ZoneRaider v4f: Empty zone list");
      return;
     }
   ...
   if(g_zoneCount > 0)
     {
      ...
      Print("ZoneRaider v4f: No actionable S/R zones");
  }
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Poll /events and store suspend windows                          |
//+------------------------------------------------------------------+
void PollEvents()
  {
   ...
   if(res != 200)
     {
      Print("ZoneRaider v4f: Events poll failed HTTP=", res);
      return;
     }
   ...
   // Parse array of {"time":"HH:MM","suspendMinutes":N,...}
   int pos = 0, len = StringLen(json);
   while(pos < len)
     {
      int objStart = StringFind(json, "{", pos);
      ...
      string timeStr = JsonGetString(obj, "time"); // "HH:MM"
      int    suspend = (int)JsonGetDouble(obj, "suspendMinutes");
      if(suspend == 0) suspend = 5; // default
...
//+------------------------------------------------------------------+
//| Return true if current UTC time is inside any suspend window   |
//+------------------------------------------------------------------+
bool IsSuspended()
  {
...
void PlaceAllOrders()
  {
   ...
   if(minRisk <= 0.0) { Print("ZoneRaider v4f: Cannot calculate min risk"); return; }
   ...
   if(oCount == 0) { Print("ZoneRaider v4f: No qualifying zones"); return; }
   ...
   for(int q = 0; q < oCount; q++)
     {
      ...
      if(alreadyPlaced) { Print("ZoneRaider v4f: Zone already placed — skipped: ", z.id); continue; }
      ...
      if(duplicate) { Print("ZoneRaider v4f: Duplicate price level skipped: ", levelKey); continue; }

      if(HasOpenAtLevel(z.direction, z.entryPrice))
        { Print("ZoneRaider v4f: Level already covered: ", z.direction, " @ ", DoubleToString(z.entryPrice, _Digits)); continue; }
   ...
   if(fCount == 0) { Print("ZoneRaider v4f: No unique uncovered zones to place"); return; }

   //--- Drop lowest-priority tail if total risk exceeds daily budget
   while(fCount > 0 && minRisk * fCount > InpMaxDailyRisk)
     { Print("ZoneRaider v4f: Dropping lowest-priority zone (budget)"); fCount--; }
   if(fCount == 0) { Print("ZoneRaider v4f: Budget too small"); return; }

   double weightSum = 0.0;
   for(int q = 0; q < fCount; q++) weightSum += filteredW[q];

   Print("ZoneRaider v4f: Placing ", fCount, " order(s)  SL=", InpSLPips,
...
void PlaceLimitOrder(const int idx, const double slDist, const double budget)
  {
   ...
   if(HasOpenAtLevel(z.direction, entry))
     {
      Print("ZoneRaider v4f: PlaceLimitOrder skip — already covered: ",
   ...
   if(!OrderSend(req, res))
     {
      Print("ZoneRaider v4f: OrderSend failed zone=", idx,
   ...
   Print("ZoneRaider v4f: [", z.strength, "] ", z.direction,
         " LIMIT #", res.order,
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
     {
      ...
      req.order  = ticket;
      if(!OrderSend(req, res))
         Print("ZoneRaider v4f: Cancel order failed #", ticket, "  code=", res.retcode);
     }
