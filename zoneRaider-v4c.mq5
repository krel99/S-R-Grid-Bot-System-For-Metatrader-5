//+------------------------------------------------------------------+
//| ZoneRaider_v4c.mq5                                              |
//| Limit entry · M5 swing trail · breakeven lock                  |
...
//--- Inputs
input string InpServerURL        = "http://37.46.211.146:3000";
input string InpSymbolName       = "";
input bool   InpAllowLong        = true;
input bool   InpAllowShort       = true;
input int    InpSLPips           = 15;
input int    InpTPPips           = 20;
input double InpMaxDailyRisk     = 100.0;
input int    InpPollMinutes      = 10;
input int    InpNYOpenHour       = 13;
input int    InpNYOpenMinute     = 30;
input int    InpEODHour          = 21;
input bool   InpSkipWeekends     = true;
input int    InpEntryBuffer      = 0;
input bool   InpSkipWatchedOnly  = true;
input int    InpSwingLookback    = 10;
input int    InpMinProfitToTrail = 10;
input int    InpSwingBuffer      = 2;
input int    InpBreakevenPips    = 8;   // Profit at which SL is moved to entry+1 pip
...
//+------------------------------------------------------------------+
//| OnTick — breakeven lock (every tick) + swing trail (new M5 bar) |
//+------------------------------------------------------------------+
void OnTick()
  {
...
//+------------------------------------------------------------------+
//| Breakeven lock — fires every tick; moves SL to entry+1pip once  |
//| profit reaches InpBreakevenPips. Idempotent: only improves SL.  |
//+------------------------------------------------------------------+
void BreakevenLock(ulong ticket)
  {
   ...
   if(profit < InpBreakevenPips * pip) return;

   double beSL;
   bool   improve;
   if(dir == "buy")
     {
      beSL    = NormalizeDouble(entry + pip, _Digits);
      improve = beSL > curSL;
     }
   else
     {
   ...
   if(!OrderSend(req, res))
      ...
      Print("ZoneRaider v4c: Breakeven locked #", ticket,
            "  SL -> ", DoubleToString(beSL, _Digits));
  }
...
void PollServer(const MqlDateTime &dt)
  {
   ...
   Print("ZoneRaider v4c: Polling ", url);
   ...
   if(g_zonesLoaded && g_zoneCount > 0)
     ...
     {
      Print("ZoneRaider v4c: No zones after NY open — done for today");
      g_ordersPlaced = true;
     }
  }
...
void ParseZones(const string json)
  {
   g_zoneCount = 0;
   ArrayResize(g_zones, 0);
   if(StringFind(json, "{") < 0) { Print("ZoneRaider v4c: Empty zone list"); return; }
   ...
   if(g_zoneCount > 0)
     {
      ...
      Print("ZoneRaider v4c: No actionable S/R zones in response");
  }
...
void PlaceAllOrders()
  {
   ...
   if(minRisk <= 0.0)
     { Print("ZoneRaider v4c: Cannot calculate min risk"); g_ordersPlaced = true; return; }
   ...
   if(oCount == 0)
     { Print("ZoneRaider v4c: No qualifying zones"); g_ordersPlaced = true; return; }

   while(oCount > 0 && minRisk * oCount > InpMaxDailyRisk)
     { Print("ZoneRaider v4c: Dropping lowest-priority zone (budget)"); oCount--; }

   if(oCount == 0)
     { Print("ZoneRaider v4c: Budget too small"); g_ordersPlaced = true; return; }

   double weightSum = 0.0;
   for(int q = 0; q < oCount; q++) weightSum += weights[q];

   Print("ZoneRaider v4c: SL=", InpSLPips, "p  TP=", InpTPPips,
         "p  BE=", InpBreakevenPips, "p  Zones=", oCount, "  Budget=$", InpMaxDailyRisk);
   ...
   g_ordersPlaced = true;
   Print("ZoneRaider v4c: Placement done. Risk committed: $", NormalizeDouble(g_dailyRiskUsed, 2));
  }
...
void PlaceLimitOrder(const int idx, const double slDist, const double budget)
  {
   ...
   if(!OrderSend(req, res))
      Print("ZoneRaider v4c: OrderSend failed zone=", idx, " retcode=", res.retcode);
   else
     {
      g_dailyRiskUsed += CalcRiskForLots(lots, slDist);
      Print("ZoneRaider v4c: [", z.strength, "] ", z.direction, " LIMIT #", res.order,
...
void CloseByTicket(ulong ticket)
  {
   ...
   if(!OrderSend(req, res))
      Print("ZoneRaider v4c: CloseByTicket failed #", ticket, " code=", res.retcode);
  }
