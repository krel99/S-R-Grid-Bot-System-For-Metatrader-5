//+------------------------------------------------------------------+
//| ZoneRaider_v4c.mq5                                              |
//| Breakeven lock at InpBEPips profit, then swing trail            |
//+------------------------------------------------------------------+
#property copyright ""
...
input int    InpTPPips            = 30;                       // Take profit (pips) — wider, BE makes this low-risk
input int    InpBEPips            = 12;                       // Pips in profit to lock breakeven (entry+1pip)
...
input int    InpMinProfitToTrail  = 20;                       // Pips in profit before trail activates (after BE)
...
int OnInit()
  {
   EventSetTimer(30);
   Print("ZoneRaider v4c: Initialized on ", _Symbol, "  magic=", g_magic,
         "  BE=", InpBEPips, "p  SL=", InpSLPips, "p  TP=", InpTPPips, "p");
   ...
   if(dt.day_of_year != g_lastDay)
     {
      ...
      Print("ZoneRaider v4c: New day — session reset");
     }
   ...
   if(!g_zonesLoaded)
     {
      ...
      if(IsAfterNYOpen(dt))
         Print("ZoneRaider v4c: WARNING — past NY open, zones not yet loaded");
      PollServer();
     }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      //--- Breakeven lock — fires every tick (improve-check makes it idempotent)
      BreakevenLock(ticket);

      //--- Swing trail fires only on new M5 bar (activates after InpMinProfitToTrail)
      ...
      if(TimeCurrent() - openTime >= 4 * 3600)
        {
         Print("ZoneRaider v4c: 4h close #", ticket);
...
void SwingTrail(ulong ticket)
  {
   ...
   if(!OrderSend(req, res))
      Print("ZoneRaider v4c: SwingTrail failed #", ticket, "  code=", res.retcode);
   else
      Print("ZoneRaider v4c: Trail #", ticket,
            "  SL ", DoubleToString(currentSL, _Digits),
            " -> ", DoubleToString(newSL, _Digits));
  }

//+------------------------------------------------------------------+
//| Breakeven lock — moves SL to entry+1pip once profit ≥ InpBEPips |
//| Safe to call every tick: improve-check skips once BE is set     |
//+------------------------------------------------------------------+
void BreakevenLock(ulong ticket)
  {
   if(!PositionSelectByTicket(ticket)) return;
   ...
   //--- Only trigger when profit >= InpBEPips
   double profit = (dir == "buy") ? bid - entry : entry - ask;
   if(profit < InpBEPips * pip) return;

   //--- Target: entry + 1 pip — small lock above true breakeven
   double beSL = (dir == "buy")
                  ? NormalizeDouble(entry + pip, _Digits)
                  : NormalizeDouble(entry - pip, _Digits);

   //--- Only move SL in improving direction — never pull it back
   bool improve = (dir == "buy") ? beSL > currentSL : beSL < currentSL;
   if(!improve) return;
   ...
   req.tp       = currentTP;
   if(!OrderSend(req, res))
      Print("ZoneRaider v4c: BreakevenLock failed #", ticket, "  code=", res.retcode);
   else
      Print("ZoneRaider v4c: Breakeven locked #", ticket,
            "  SL ", DoubleToString(currentSL, _Digits),
...
void PollServer()
  {
   ...
   if(res != 200)
     {
      Print("ZoneRaider v4c: Poll failed HTTP=", res,
...
void ParseZones(const string json)
  {
   ...
   if(StringFind(json, "{") < 0)
     {
      Print("ZoneRaider v4c: Empty zone list");
      return;
     }
   ...
   if(g_zoneCount > 0)
     {
      g_zonesLoaded = true;
      Print("ZoneRaider v4c: Loaded ", g_zoneCount, " zone(s)");
     }
   else
      Print("ZoneRaider v4c: No actionable S/R zones");
  }
...
void PlaceAllOrders()
  {
   ...
   if(minRisk <= 0.0) { Print("ZoneRaider v4c: Cannot calculate min risk"); return; }
   ...
   if(oCount == 0) { Print("ZoneRaider v4c: No qualifying zones"); return; }
   ...
   for(int q = 0; q < oCount; q++)
     {
      ...
      if(alreadyPlaced) { Print("ZoneRaider v4c: Zone already placed — skipped: ", z.id); continue; }
      ...
      if(duplicate) { Print("ZoneRaider v4c: Duplicate price level skipped: ", levelKey); continue; }

      if(HasOpenAtLevel(z.direction, z.entryPrice))
        { Print("ZoneRaider v4c: Level already covered: ", z.direction, " @ ", DoubleToString(z.entryPrice, _Digits)); continue; }
   ...
   if(fCount == 0) { Print("ZoneRaider v4c: No unique uncovered zones to place"); return; }

   //--- Drop lowest-priority tail if total risk exceeds daily budget
   while(fCount > 0 && minRisk * fCount > InpMaxDailyRisk)
     { Print("ZoneRaider v4c: Dropping lowest-priority zone (budget)"); fCount--; }
   if(fCount == 0) { Print("ZoneRaider v4c: Budget too small"); return; }

   double weightSum = 0.0;
   for(int q = 0; q < fCount; q++) weightSum += filteredW[q];

   Print("ZoneRaider v4c: Placing ", fCount, " order(s)  SL=", InpSLPips,
...
void PlaceLimitOrder(const int idx, const double slDist, const double budget)
  {
   ...
   if(HasOpenAtLevel(z.direction, entry))
     {
      Print("ZoneRaider v4c: PlaceLimitOrder skip — already covered: ",
   ...
   if(!OrderSend(req, res))
     {
      Print("ZoneRaider v4c: OrderSend failed zone=", idx,
   ...
   Print("ZoneRaider v4c: [", z.strength, "] ", z.direction,
         " LIMIT #", res.order,
...
void CloseAndReset()
  {
   for(int i = OrdersTotal() - 1; i >= 0; i--)
     {
      ...
      req.order  = ticket;
      if(!OrderSend(req, res))
         Print("ZoneRaider v4c: Cancel order failed #", ticket, "  code=", res.retcode);
     }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetInteger(POSITION_MAGIC) != g_magic)  continue;
      CloseByTicket(ticket);
      Print("ZoneRaider v4c: EOD closed #", ticket);
     }
   ...
   ArrayResize(g_zones, 0);
   ArrayResize(g_placedIds, 0);
   Print("ZoneRaider v4c: EOD complete");
  }
