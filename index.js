...
// EA: active zones for a symbol.
// Regular zones: today only, before cutoff.
// NWOG zones: current week, no cutoff (valid until Friday market close).
app.get("/:symbol", (req, res) => {
  const symbol = req.params.symbol.toUpperCase();
  const today = todayDate();
  const weekStart = getWeekStart();
  const result = readZones().filter((z) => {
    if (z.symbol !== symbol) return false;
    if (z.direction === "nwog") return z.date >= weekStart;
...
app.post("/zones", (req, res) => {
  ...
  if (!symbol || !direction) return res.status(400).json({ error: "Missing required fields" });
  if (!["buy", "sell", "above", "below", "nwog"].includes(direction))
    return res.status(400).json({ error: "direction must be buy, sell, above, below, or nwog" });

  // ── NWOG: one zone per symbol per week, bidirectional ────────────────────
  if (direction === "nwog") {
    if (from == null || to == null) return res.status(400).json({ error: "from and to required for nwog" });
    const sym = symbol.toUpperCase();
    // Replace any existing NWOG for this symbol
    const existing = readZones().filter((z) => !(z.symbol === sym && z.direction === "nwog"));
    const zone = {
      id: uuidv4(),
      symbol: sym,
      direction: "nwog",
  ...
  // ── Regular zones ─────────────────────────────────────────────────────────
  if (!strength || !kind) return res.status(400).json({ error: "Missing required fields" });
