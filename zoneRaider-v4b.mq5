...
void PlaceAllOrders()
  {
   ...
   if(slotsLeft <= 0) { Print("ZoneRaider v4b: Max positions/orders reached"); return; }
   ...
   while(oCount > 0 && minRisk * oCount > InpMaxDailyRisk)
     { Print("ZoneRaider v4b: Dropping lowest-priority zone (budget)"); oCount--; }
   if(oCount == 0) { Print("ZoneRaider v4b: Budget too small"); return; }

   double weightSum = 0.0;
   for(int q = 0; q < oCount; q++) weightSum += weights[q];

   Print("ZoneRaider v4b: Placing ", oCount, " order(s)  SL=", InpSLPips,
